# -*- coding: utf-8 -*-
"""
Created on Fri May 15 11:10:22 2026

@author: Enzo
"""

###############################################################################
## PROYECTO PARA ELABORAR UNA CARACTERIZACIÓN DE SUBVENCIONES VINCULADAS A LA
## BIOTECNOLOGÍA CONSIDERANDO EL OBSERVATORIO DE SUBVENCIONES DEL PROCIENCIA
###############################################################################
...
observa = pd.read_excel("Subvenciones_12may.xlsx", sheet_name="Resultados", header=0)
...
###############################################################################
# Se implementa un algoritmo de búsqueda para construir un subset del dataframe
# observa
###############################################################################

# Este subset estará integrado por todas las subvenciones vinculadas con la biotecnología
...
# Se consideran las palabras clave relacionadas estrechamente con la biotecnología
patron = r"biotecnologia|biotecnolog|Bioinformatica|Biologia celular|microbiologia|bioquimica|biologia molecular|genetica|herencia|nanomateriales|nanoprocesos|biotecnologia para la salud|biotecnologia agropecuaria|biotecnologia acuicola|biotecnologia ambiental|biotecnologia de manufactura|bioproductos|biotecnologia sintetica|neurotecnologia|crispr|biotech"

df_biotech = df_busqueda[
...
# Se calcula la cantidad de subvenciones durante el periodo de análisis
df_biotech["ID_CONTRATO"].count()

# Se calcula la suma del total de subvenciones durante el periodo de análisis
df_biotech["MONTO"].sum()
...
df_biotech_año = (df_biotech.groupby("AÑO", as_index=False).agg({"MONTO":"sum", "ID_CONTRATO":"count"}))
# Se renombra la columna ID_CONTRATO del dataframe df_biotech_año
df_biotech_año.rename(columns=({"ID_CONTRATO":"CANTIDAD"}), inplace=True)


df_plot = df_biotech_año.copy()
...
obv_año_estado = pd.pivot_table(df_biotech, values="ID_CONTRATO", index="AÑO", columns="ESTADO", aggfunc="count")
...
###############################################################################
biotech_convenio = df_biotech.groupby("CONVENIO")["ID_CONTRATO"].count()
biotech_convenio = biotech_convenio.to_frame()
biotech_convenio.reset_index(inplace=True)
biotech_convenio.columns

# Se renombra algunas columnas del dataframe biotech_convenio
biotech_convenio.rename(columns=({"ID_CONTRATO":"CANTIDAD"}), inplace=True)
biotech_convenio = biotech_convenio[biotech_convenio["CANTIDAD"]>=4]

# ----------------------------
# Datos
# ----------------------------
biotech_convenio = biotech_convenio.sort_values(
    "CANTIDAD", ascending=False).reset_index(drop=True)

# Etiquetas abreviadas para evitar saturación
biotech_convenio["LABEL"] = biotech_convenio["CONVENIO"].replace({
...
sizes_scaled = np.sqrt(biotech_convenio["CANTIDAD"]) * 180
...
x = x[:len(biotech_convenio)]
y = y[:len(biotech_convenio)]
...
colors = [color_secundario] * len(biotech_convenio)
if len(biotech_convenio) > 0:
    colors[0] = color_principal
if len(biotech_convenio) > 1:
...
# Etiquetas
# ----------------------------
for i, row in biotech_convenio.iterrows():
...
# según intervención
###############################################################################
biotech_inter = df_biotech.INTERVENCIÓN.value_counts()
biotech_inter = df_biotech.INTERVENCIÓN.value_counts(normalize=True).round(2)*100
biotech_inter = biotech_inter.to_frame()
biotech_inter.reset_index(inplace=True)
biotech_inter.rename(columns=({"proportion":"porcentaje"}), inplace=True)


biotech_inter = biotech_inter.sort_values("porcentaje", ascending=True)

color_principal = "#0B4F6C"   # azul institucional
color_secundario = "#5FB7C6"  # celeste

colors = [color_secundario] * len(biotech_inter)
colors[-1] = color_principal  # destacar el mayor

plt.figure(figsize=(12, 6))

bars = plt.barh(biotech_inter["INTERVENCIÓN"], biotech_inter["porcentaje"], color=colors)

# Etiquetas
for i, v in enumerate(biotech_inter["porcentaje"]):
...
barra_apilada = pd.pivot_table(df_biotech, values="MONTO", index="INTERVENCIÓN", columns="AÑO", aggfunc="sum")
...
###############################################################################
# Se realiza un gráfico para identificar quienes investigan en biotecnología en el Perú
# por investigador y por universidad
################################################################################

# Por investigador 
biotech_investigador = df_biotech.LIDER_PROYECTO.value_counts()
biotech_investigador = biotech_investigador.to_frame()
biotech_investigador.reset_index(inplace=True)
biotech_investigador.rename(columns=({"count":"Cantidad"}), inplace=True)
biotech_investigador["Cantidad"] = biotech_investigador["Cantidad"].astype(int)
biotech_investigador = biotech_investigador.head(10)
biotech_investigador.dtypes

biotech_investigador = biotech_investigador.sort_values("Cantidad", ascending=True)

color_principal = "#0B4F6C"   # azul institucional
color_secundario = "#5FB7C6"  # celeste

colors = [color_secundario] * len(biotech_investigador)
colors[-1] = color_principal  # destacar el mayor

plt.figure(figsize=(12, 6))

bars = plt.barh(biotech_investigador["LIDER_PROYECTO"], biotech_investigador["Cantidad"], color=colors)

# Etiquetas
for i, v in enumerate(biotech_investigador["Cantidad"]):
...
# Por universidad
biotech_uni = df_biotech[df_biotech["ORGANIZACIÓN"].str.contains("UNIVERSIDAD", case=False, na=False)]
biotech_uni = biotech_uni.ORGANIZACIÓN.value_counts(normalize=True).round(3)*100
biotech_uni = biotech_uni.to_frame()
biotech_uni.reset_index(inplace=True)
biotech_uni.rename(columns=({"proportion":"Porcentaje"}), inplace=True)
biotech_uni = biotech_uni.head(10)

biotech_uni_a = biotech_uni.sort_values("Porcentaje", ascending=True)

color_principal = "#0B4F6C"   # azul institucional
color_secundario = "#5FB7C6"  # celeste

colors = [color_secundario] * len(biotech_uni_a)
colors[-1] = color_principal  # destacar el mayor

plt.figure(figsize=(12, 6))

bars = plt.barh(biotech_uni_a["ORGANIZACIÓN"], biotech_uni_a["Porcentaje"], color=colors)

# Etiquetas
for i, v in enumerate(biotech_uni_a["Porcentaje"]):
    plt.text(v + 0.5, i, f"{v:.0f}%", va="center", fontsize=10)
...
df_biotech["PRODUCCION"] = df_biotech["PUB"] + df_biotech["TESIS"] + df_biotech["PAT"]

# Se eliminan aquellos registros que presentan valor igual a 0 del campo PRODUCCION
df_biotech = df_biotech[df_biotech["PRODUCCION"]!=0]

# se consideran los proyectos que se encuentran en condición de concluido
observa_conclu = df_biotech[df_biotech["ESTADO"]=="Concluido"]
...
universidad = df_biotech[df_biotech["ORGANIZACIÓN"].str.contains("UNIVERSIDAD", case=False, na=False)]
...
universidad_beca = universidad_beca.sort_values("MONTO", ascending=False)
universidad_beca = (universidad_beca.groupby("ORG_SHORT", as_index=False).agg({"MONTO":"sum"}))

# Convertir a millones
universidad_beca["MONTO_M"] = universidad_beca["MONTO"] / 1e6

# Colores profesionales
colors = [
    "#D9D9D9", "#5FB7C6", "#A3AD2C"
]

fig, ax = plt.subplots(figsize=(9, 7))

wedges, texts, autotexts = ax.pie(
    universidad_beca["MONTO_M"],
    labels=universidad_beca["ORG_SHORT"],
    autopct="%1.1f%%",
    startangle=90,
    colors=colors[:len(universidad_beca)],
    pctdistance=0.78,
    labeldistance=1.08,
    wedgeprops={
        "width": 0.42,
...
# Centro del donut
total_m = universidad_beca["MONTO_M"].sum()
